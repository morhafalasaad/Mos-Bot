...
def _generate(
    ...
    queued items — see retry_pending_github_queue) — deliberately NOT
...
def generate_with_outer_backoff(prompt: str, **kwargs):
    ...
    retry_pending_github_queue()). That's "wait for the RPM window to
...
class TokenUsageTracker:
    """
    Lightweight, fail-safe, SILENT analytics logger. Appends ONE consolidated
    JSON record per fully-processed project to config.TOKEN_USAGE_STATS_FILE
    — not one record per raw Gemini call (a project can involve up to two
    calls: scoring, and optionally proposal drafting; their token counts and
    response times are summed into a single row by record_token_usage()
    below, since main.py needs to add sent_to_telegram AFTER both calls and
    the notification decision are already done).
    ...
    Any file read/write error (disk full, permissions, corrupted existing
    file, etc.) is caught and discarded without a trace, because the one
    hard requirement here is that a broken stats file must NEVER interrupt
    the evaluation worker. If you need to debug this class, temporarily add
    logging yourself — by design it stays out of the way otherwise.
    """

    def __init__(self, path: str = None):
        self.path = path or config.TOKEN_USAGE_STATS_FILE
    ...
    def record(
        ...
        try:
            entry = {
                ...
                # None -> JSON null, exactly for the "or null if failed" case
