...
def retry_pending_queue():
    """
    Reads the MongoDB-hosted pending-projects retry queue (see db.py) and
    attempts to re-evaluate each entry with Gemini.
    ...
    - Still failing (still ai_failed, including a proactive rate-limit
      ...
      incremented via a single-field MongoDB update (no need to
      read-modify-write the whole queue the way the old GitHub-JSON
      version had to).
    - Exceeds config.GITHUB_QUEUE_MAX_RETRIES: dropped from the auto-retry
      queue (still preserved in the human-readable GitHub Issue/file
      record, if GitHub fallback is configured, from when it was first
    ...
    logger.info("Found %s pending project(s) in the MongoDB retry queue", len(pending))
    ...
    for entry in pending:
        ...
        if evaluation is None or evaluation.ai_failed:
            ...
            if retry_count > config.GITHUB_QUEUE_MAX_RETRIES:
                ...
                db.remove_pending_project(project_id)
                continue
            db.update_pending_project(project_id, {"retry_count": retry_count})
        ...
        if evaluation.match_score >= evaluation.effective_threshold and evaluation.proposal_ar:
            repost_warning = repost_detector.check_and_record(
                project_id, title, entry.get("description", ""),
            )
...
def retry_open_github_issues():
    ...
    independent of the MongoDB queue above — a safety net for "orphaned"
    issues whose queue entry is missing for any reason (e.g. the MongoDB
    ...
    (those are handled by retry_pending_queue() above, which closes
    ...
    tracked_issue_numbers = {
        ...
        for entry in db.get_pending_projects()
        if entry.get("issue_number")
    }
...
def handle_ai_unavailable(project, reason: str):
    ...
    alert, saves the OPTIONAL human-readable GitHub record (if configured),
    and queues it in MongoDB for automatic re-evaluation. Never raises.
    """
    ...
    queued = db.queue_pending_project({
        "id": project.id,
        "title": project.title,
        "description": project.description,
        "url": project.url,
    ...
    logger.warning(
        "AI evaluation unavailable for '%s' — GitHub record saved: %s (issue #%s) | queued in MongoDB for auto-retry: %s",
...
# the MongoDB retry backlog.
# ---------------------------------------------------------------------------

def consumer_loop():
    logger.info(
        "Consumer starting — retry backlog checked every ~%ss, "
        "per-task timeout %ss",
    ...
    while True:
        ...
        try:
            now = time.time()
            if now - last_retry_check >= config.GITHUB_RETRY_CHECK_INTERVAL:
                retry_pending_queue()
                retry_open_github_issues()
                last_retry_check = now
        except Exception:
            logger.error("Consumer's retry-backlog check failed:\n%s", traceback.format_exc())
...
_TELEGRAM_OFFSET_STATE_KEY = "telegram_update_offset"


def _load_telegram_offset() -> int:
    """Reads back the last-processed Telegram update_id + 1 from MongoDB
    (see db.py), so a restart doesn't reprocess (and double-toast) already-
    handled button taps. Returns 0 (process from whatever Telegram
    currently has queued) on ANY failure — missing document, a Mongo
    hiccup, or anything else — never raises, matching the fail-safe
    convention used by every other persistence helper in this codebase
    (ScoreCache, DailyRequestTracker, outcome_tracker, repost_detector)."""
    try:
        return int(db.get_state(_TELEGRAM_OFFSET_STATE_KEY, 0))
    except Exception:
        return 0
...
def _save_telegram_offset(offset: int):
    ...
    not a crash. db.set_state already never raises on its own, this
    wrapper is defense-in-depth only, same fail-safe convention as every
    other persistence helper in this codebase (ScoreCache,
    DailyRequestTracker, outcome_tracker, repost_detector)."""
    try:
        db.set_state(_TELEGRAM_OFFSET_STATE_KEY, offset)
    except Exception:
        pass
