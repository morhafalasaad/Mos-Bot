...
#
# Storage is MongoDB Atlas's `seen_projects` collection (see db.py) as of
# the Sept 2026 migration off a local JSON file — durable across Render
# restarts/redeploys, unlike the old local-disk version, and pruned
# automatically by a TTL index (config.SEEN_PROJECTS_TTL_DAYS) rather than
# the old "keep the last 500" manual trim.
# ---------------------------------------------------------------------------

def _load_seen() -> set:
    return db.get_seen_ids()


def _save_seen(seen_ids) -> None:
    db.mark_seen(seen_ids)
...
def get_new_projects() -> List[Project]:
    ...
    try:
        ...
        # Also exclude anything already sitting in the MongoDB retry queue
        # (see db.get_pending_projects) — a project can land there without
        # ever being added to `seen` if, say, the process restarted
        # between scraping it and marking it seen, and we don't want it
        # evaluated a SECOND time here while main.py's retry loop ALSO
        # processes it from the queue (duplicate Telegram messages for one
        # project). Unlike the old local-JSON `seen` set this used to need
        # a GitHub-hosted cross-check to compensate for, `seen` itself is
        # now durable (MongoDB), so this is just a belt-and-suspenders
        # check against the other durable store, not a redeploy workaround.
        seen = _load_seen()
        try:
            pending_ids = {entry.get("id") for entry in db.get_pending_projects() if entry.get("id")}
        except Exception as exc:
            logger.warning("Could not check MongoDB pending queue for dedup: %s", exc)
            pending_ids = set()

        new_projects = [
            p for p in all_projects
            if p.id not in seen and p.id not in pending_ids
        ]
        if pending_ids:
            logger.info(
                "Excluded %s project(s) already tracked in the retry "
                "queue from this cycle's 'new' list",
                len(set(p.id for p in all_projects) & pending_ids),
            )
